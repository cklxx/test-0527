...
async fn main() {
    let (mut layout, mut noder) = mock_layout("foo.html");
    let mut frame = 0.;
    let start = now();
    loop {
        if frame == 60. {
            (layout, noder) = mock_layout("bar.html");
        }
        frame += 1.;
        layout
...
impl Layout {
    ...
    fn render_layout(
        ...
        draw_text(&text, x, y + 50.0, 20.0, fcolor);
        if let Ok(nodes) = self.stretch.children(*root) {
            for node in nodes {
                self.render_layout(&node, root_layout.location).unwrap();
            }
        }
        Ok(())
    }
        ...
        match node.data {
            ...
            NodeData::Text { ref contents } => {
                self.store.insert(
                    vnode,
                    VAttributes {
                        text: trim_text(contents.borrow().escape_default().to_string()),
                        ..Default::default()
                    },
                );
            }
        ...
        for child in node
            ...
            .filter(|child| match &child.data {
                NodeData::Text { contents } => {
                    let mut result = true;
                    let mut text_temp = contents.borrow().escape_default().to_string();
...
fn mock_layout(path: &str) -> (Layout, stretch::node::Node) {
